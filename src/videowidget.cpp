...
{
    return FrameAnnotation::displaySizeForRotation(QSize(m_videoWidth, m_videoHeight), m_displayRotation);
}

/// @brief 存储系（原视频坐标）→ 显示系（旋转后）。与 QTransform().rotate(档位) 逐点一致。
QPoint OverlayWidget::storedToDisplay(const QPoint &p) const
{
    return FrameAnnotation::rotateStoredToDisplay(p, QSize(m_videoWidth, m_videoHeight), m_displayRotation);
}
...
void OverlayWidget::paintEvent(QPaintEvent *event)
{
    ...
    if (m_magnifierRect.isValid()) {
        ...
        if (rc.isValid() && !rc.isEmpty())
            FrameAnnotation::drawMagnifierIndicator(painter, rc, m_magnifierZoom);
    }
}
