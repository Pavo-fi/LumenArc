...
void MainWindow::createMenus()
{
    ...
    helpMenu->addAction(lang("使用手册", "User Manual"), []() {
        QString path = QCoreApplication::applicationDirPath() + "/LumenArc_v0.3_操作手册.pdf";
...
void MainWindow::createToolBar()
{
    ...
    m_loopBtn->setToolTip(lang("循环播放 A-B 区域 (L)", "Loop A-B region (L)"));
...
void MainWindow::onDurationChanged(qint64 durationMs)
{
    ...
    if (!m_currentVideoPath.isEmpty() && m_videoListPanel) {
        m_videoListPanel->updateDuration(m_currentVideoPath, durationMs);
    }
