...
| 本次 | 辅助线 UX 重构 | ① 去掉 QInputDialog 弹框：右键菜单"添加水平/垂直辅助线"直接取鼠标位置换算（水平=鼠标 Y→亮度值，垂直=鼠标 X→时间，垂直原先用播放光标时间）② 标签文本移入 drawChartGuideLines 每次 draw 刷新 → 拖动时数值实时联动（原只在创建时设置）③ 水平辅助线双标签：左侧=亮度值（左 Y 轴），右侧=响度 dB 值（右 Y 轴同一像素等效值，无音量轴时隐藏） |
