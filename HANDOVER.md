...
| 版本 | 项目 | 内容 | 前置依赖 | 预估 | 价值 |
|---|---|---|---|---|---|
| **v2.0** | **案件模块** | 案件=证据容器：视频、ROI/标签/辅助线、分析结果、前处理产物、证据报告统一入案；案件列表/归档/打包移交；.vla 关联案件存储（触及数据组织，属大架构） | 无 | 2-3 周 | 证据链完整性；换机/移交不再丢资料；多案件并行管理 |
| **v2.1** | **分析报告模块** | 一键生成标准分析报告：案件信息 + 时间线 + 亮度曲线 + 音频分析 + 截图/标签 + 前处理证据 + 结论栏，PDF 输出（含盖章位） | 案件模块 | 1.5-2 周 | **最终交付物**：报告从手工拼装（小时级）到一键生成，格式统一可提交 |

### 第二梯队：证据核心（重要性 ★★★★★ / 紧迫性 ★★★★）

| 版本 | 项目 | 内容 | 前置依赖 | 预估 | 价值 |
|---|---|---|---|---|---|
| **v2.2** | **视频校时深化**（P4 OCR 的深化） | 前处理 OCR 结果反哺：相机时钟偏差检测（OSD vs 真实时间）、多机时间线对齐、图表"时间设置"自动校准替代手输 | 现有前处理 OCR + P1a 任务框架 | 1-2 周 | 时间线是火调证据骨架；自动对时消除手输误差 |
...
| 前处理 v2 | 硬件编码/重叠裁剪/多组并行/智能命名 | NVENC/QSV（参数等价性评审）；OCR 时间戳驱动的重叠段自动剪切；多通道并行流水线；"通道_日期_起止时间"自动命名 | — | 1-2 周 | 转码 20x+（4090）；素材准备全流程自动化 |

### 第四梯队：架构维护（重要性 ★★★ / 紧迫性 ★★）

| 版本 | 项目 | 内容 | 前置依赖 | 预估 | 价值 |
|---|---|---|---|---|---|
| v1.7 | P1a/P1b 任务化 + 通道化 | TaskRegistry 分析功能注册制 + AnalysisSnapshot 通道字典 + .vla v7 格式 | 无 | 2 周 | 新分析类型/OCR 租户前置 |
...
**排期说明**：案件/报告/校时直接服务"交付"（火调办案终点是可提交的报告），价值密度高于 GPU/引擎技术红利；校时依赖前处理 OCR 已铺好的基础（probe_timestamps.py），属深化而非从零；技术红利解决"更快/更省"（内部效率），故靠后。
