...
# ============================================================================
# 工作记录（2026-08-15，第二十四批）——输出路径自选 + 音量增强 + 辅助线配色
# ============================================================================

## 33. 三项需求实施（用户拍板）

### 1. 前处理输出路径自选
- 案件导入模式下放开输出目录编辑/浏览（原拍板§8-11 禁手改，现更新）
- browse 校验：案内目录允许；案外拒绝并提示「切换独立输出」
- addPreprocessSession 案外拒绝兜底不变

### 2. 音频音量增强（最大 500%）
- 引擎 setVolume 上限 100 → 500；≤100% 走 sink 硬件音量（行为不变）
- >100%：sink 音量封顶 100% + PCM 软件增益（applyVolumeGain 按 sink
  样本格式分支 Int16/Int32 限幅、Float 钳 -1..1、UInt8 偏移限幅——削波保护）
- ↑/↓ 步进 ±5、上限 500；**破 200% 时弹一次提示**（每会话一次防打扰）
- 状态栏显示改为百分比（现音量：205%）

### 3. 图表 Y 轴辅助线配色
- 水平辅助线默认色：黄(255,255,0,180) → 浅蓝 #3E9BD8（语谱底噪滑块
  同色 Theme::Info）不透明度 70%（alpha 178）
- ChartGuideData 默认色同步（新保存数据走新色）；垂直线青色不变

### 验证
构建零错误；全回归绿（ui_chain 92 / case 242 / preprocess 170 / vla 13 等）。
待真机：↑ 键到 205% 看提示与音量、辅助线新色、案件模式 browse 案外拒绝。
