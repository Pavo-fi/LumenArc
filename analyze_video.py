...
def analyze_single_video(video_path, rois, ffmpeg, processes=1,
    ...
    if processes > 1:
        ...
        # Split into fine-grained chunks (4x process count) so that progress
        # can be reported incrementally as chunks complete.
        chunk_count = processes * 4
        seg_size = max(1, (end - start) // chunk_count)
        segments = []
        rois_json_str = json.dumps(rois, separators=(',', ':'))
        for i in range(chunk_count):
            s = start + i * seg_size
            e = start + (i + 1) * seg_size if i < chunk_count - 1 else end
            if s < e:
        ...
        with Pool(processes) as pool:
            ...
            # imap_unordered yields each chunk's result as soon as it finishes,
            # giving real-time progress instead of a single burst at the end.
            for r in pool.imap_unordered(_segment_worker, segments):
                results.append(r)
                done += 1
