...
|---|---|---|---|
| 当前 v1.1.1 | **v7**（已实现） | — | HANDOVER 第十章所写 "v1.8.0 落地 v7" 为笔误，v7 已在库 |
| v1.2.0 | **v8**（+time_calibration） | — | F2 三件套；**本轮不写兼容字段，严格升版**（Q-19 拍板） |
| v1.3.0 | v8 不变 | **v1** | .vla 保持独立可用，案件仅持引用 |
| v1.4.0 | v8 不变 | v1 不变 | 报告产物入案件 reports/ |
| v1.8.0 | **v9**（通道化字典） | v1 兼容（vla 版本号透传） | 迁移 v7→v8→v9 链式 |
