...
证据②' 流内绝对起始墙钟（DHAV 等私有容器） 权重 0.6  [v1.2 新增]
证据③ creation_time（ISO8601 解析+脏值过滤） 权重 0.5
证据④ 文件 mtime（兜底）              权重 0.2（强制人工确认标记）
```

**证据②' 流内绝对起始墙钟（v1.2 现场反馈新增）**：Dahua DHAV 等私有容器由
录像机固件写入录制时刻（`AVFormatContext.start_time`，epoch 秒）。仅接受
2000-01-01 ~ 当前+1天 的合理区间（防止普通容器损坏文件的垃圾 PTS 误判）。
**时区约定**：DVR 把本地墙钟当作 UTC 秒写入（OSD 墙钟同基准），与 OCR 的
本地墙钟语义对齐——取 UTC 分量按本地时间重解释（实测：1784700002 →
OSD 06:00:02 ✓）。与 OCR 交叉校验（偏差 > 2min 出 EvidenceConflict 警告，
不改序）。持有该证据的组不标记 suspicious（视为有效证据，权重 0.6）。
Dahua 导出文件名 `HH.MM.SS-HH.MM.SS[R][...].dav` 为纯时间段（无日期/通道），
刻意不做文件名解析。
