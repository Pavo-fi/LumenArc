...
## 函数量（5 个）：上述四函数 + updateProfile（账号管理-修改资料，token+access_token 双校验）

## 资源点消耗实锤（2026-08-29 联调期）
- **SMS 短信 = 唯一大头：50 资源点/条**（12 条联调测试烧 600 点）；函数调用/DB 读写合计 ≈5 点
- 正常运营：每用户每月 ≈1 条短信（30 天重登一次）→ 体验版 3000 点 ≈ 支撑 60 个活跃用户
- 升级线：用户量到几十再升个人版（19.9 元/月，4 万点）；短信费 0.045 元/条另计（按量）
