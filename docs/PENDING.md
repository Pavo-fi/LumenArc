...
| P-57 | **多视频同步播放（多机时间线合并）**调研：方案 v0.2 已部分拍板（2026-08-18 用户：2-4 路/独立大窗口/2 路界面允许临时进（会话级手动对齐不落盘）/含临时进路走分开进度条模式（纯带时间进度条，无亮度音量语谱）/纯视频不看亮度）——docs/MULTICAM_PLAYBACK_TECH_DESIGN_CN.md；余项待确认 U-1~U-6（临时对齐交互/音频策略/1440p 性能闸门/游标手感/瓦片 OSD/回单路跳转） | §51 用户提需 | ◐ 部分拍板，余项确认中 |
