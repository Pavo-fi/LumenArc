...
| P-55 | **亮度分析闪退**（真机 2026-08-18 用户反馈：点击亮度分析后闪退，其余功能未见异常）——本地排查：offscreen 全链路复现（合成 H.264 素材，打开视频→缓存 vla→ROI 恢复→onAnalyze→完成）27 断言全过**无法复现**；已加固：AnalysisSnapshot 元类型显式注册（跨线程信号契约）、引擎结果行长度防御截断；待用户提供：①About 版本号 ②崩溃时机（点击即崩/分析中/完成时）③Windows 事件查看器→应用程序日志故障模块 ④素材分辨率/编码/时长 | §49 排查 | ☐ 待真机复现信息 |
