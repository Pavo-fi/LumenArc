...
| P-55 | **亮度分析闪退**——✅ **已根除（§50）**：用户提供稳定复现条件（案件模式+明景拼接视频+矩形/多边形各一）后本地用原素材实复现。根因：该文件音轨带 956ms 负时间戳 AAC priming 前导包，吃满 find_stream_info 的 500ms 探测窗口（§43 卡顿优化限制）→ 视频 codecpar pix_fmt 未知 → 引擎拿 AV_PIX_FMT_NONE 喂 sws_getContext → libswscale av_assert(desc) abort 闪退（与 ROI 类型无关，纯矩形同样崩）。修复：analyzeLuminanceOne 改按解码帧实际 width/height/format 惰性建表（对齐播放引擎既有模式），帧属性中途变化重建；正常素材行为零变化。验证：真实明景素材引擎级 35969 点×2 行 72s 完成 + mw_test 全链路 27 断言绿 + libav 套件新增 P-55 回归（合成音轨前导素材确定性复现，修复前 abort/修复后通过）；全回归 12 套绿。剩余：真机点检新构建确认 | §49 排查→§50 根除 | ✅ 已修复（待真机点检勾销） |
